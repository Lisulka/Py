# Pre-required modules = pywin32, re, pandas and datetime
...
# After data collection is done it then exports to Excel file for future analysis.
# Wrote by J.S 
# Enjoy
...
for srcip, values in emails.items():
    for i in range(len(values['date'])):
        data.append([srcip, values['srccountry'][i], values['date'][i], values['time'][i], values['sessionid'][i], values['attack'][i], values['dstip'][i]])
df = pd.DataFrame(data, columns=["Source IP", "Source Country", "Date", "Time", "Session ID", "Attack Method", "Destination IP"])

# Generate the current date
current_date = datetime.now().strftime("%d-%m-%y")

# Save the DataFrame to an Excel file
excel_filename = str(current_date)+"_FortiGate_Blacklist.xlsx"
